...
class ContainerCart {
    ...
    async getCarts() {
        if (fs.existsSync(this.pathCarts)) {
            try {
                const carts = JSON.parse(await fs.promises.readFile(this.pathCarts, 'utf-8'));
                return {
                    status : 'success',
    ...
    async createCart() {
        if (fs.existsSync(this.pathCarts)) {
            ...
            if (!carts) {
                ...
                try {
                    await fs.promises.writeFile(this.pathCarts, JSON.stringify([newCart], null, 2));
                    return {
                        status : 'success',
                        message : 'Cart add Correctly',
                        payload : newCart
    ...
    async deleteCart(id) {
        if (fs.existsSync(this.pathCarts)) {
            ...
            if (idExist) {
                ...
                try {
                    await fs.promises.writeFile(this.pathCarts, JSON.stringify(newsCarts, null, 2));
                    return {
                        status : 'success',
    ...
    async addProductToCart ( id, idProduct ) {
        if (fs.existsSync(this.pathCarts)) {
            ...
            const products = JSON.parse(await fs.promises.readFile(this.pathProducts, 'utf-8'));
            ...
            if (idExist && idExistProduct) {
                ...
                try {
                    await fs.promises.writeFile(this.pathCarts, JSON.stringify(cartsUpdated, null, 2));
                    return {
                        status : 'success',
